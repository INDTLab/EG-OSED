...
def image_data_augmentation(mat,mat1, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp, gaussian_noise, blur,
                            truth):
    try:
        img = mat
        #print(img.shape)
        ...
        if (src_rect[0] == 0 and src_rect[1] == 0 and src_rect[2] == img.shape[0] and src_rect[3] == img.shape[1]):
            ...
            sized1 = cv2.resize(img1, (w, h), cv2.INTER_LINEAR)
        else:
            cropped = np.zeros([sheight, swidth, 3])
            cropped1 = np.zeros([sheight, swidth, 3])
            cropped[:, :, ] = np.mean(img, axis=(0, 1))
            cropped1[:, :, ] = np.mean(img1, axis=(0, 1))

            cropped[dst_rect[1]:dst_rect[3], dst_rect[0]:dst_rect[2]] = \
                img[new_src_rect[1]:new_src_rect[3], new_src_rect[0]:new_src_rect[2]]
            cropped1[dst_rect[1]:dst_rect[3], dst_rect[0]:dst_rect[2]] = \
                img1[new_src_rect[1]:new_src_rect[3], new_src_rect[0]:new_src_rect[2]]

            # resize
            sized = cv2.resize(cropped, (w, h), cv2.INTER_LINEAR)
            sized1 = cv2.resize(cropped1, (w, h), cv2.INTER_LINEAR)
        if not torch.isfinite(torch.max(torch.from_numpy(sized))) or not torch.isfinite(torch.max(torch.from_numpy(sized1))):
            print("unfinite data created by cv2.resize()...")
        # flip
        if flip:
            # cv2.Mat cropped
            sized = cv2.flip(sized, 1)  # 0 - x-axis, 1 - y-axis, -1 - both axes (x & y)
            sized1 = cv2.flip(sized1, 1)
...
def blend_truth_mosaic(out_img,out_img1, img,img1, bboxes, w, h, cut_x, cut_y, i_mixup,
    ...
    if i_mixup == 0:
        ...
        out_img1[:cut_y, :cut_x] = img1[top_shift:top_shift + cut_y, left_shift:left_shift + cut_x]
    if i_mixup == 1:
        bboxes = filter_truth(bboxes, cut_x - right_shift, top_shift, w - cut_x, cut_y, cut_x, 0)
        out_img[:cut_y, cut_x:] = img[top_shift:top_shift + cut_y, cut_x - right_shift:w - right_shift]
        out_img1[:cut_y, cut_x:] = img1[top_shift:top_shift + cut_y, cut_x - right_shift:w - right_shift]
    if i_mixup == 2:
        bboxes = filter_truth(bboxes, left_shift, cut_y - bot_shift, cut_x, h - cut_y, 0, cut_y)
        out_img[cut_y:, :cut_x] = img[cut_y - bot_shift:h - bot_shift, left_shift:left_shift + cut_x]
        out_img1[cut_y:, :cut_x] = img1[cut_y - bot_shift:h - bot_shift, left_shift:left_shift + cut_x]
    if i_mixup == 3:
        bboxes = filter_truth(bboxes, cut_x - right_shift, cut_y - bot_shift, w - cut_x, h - cut_y, cut_x, cut_y)
        out_img[cut_y:, cut_x:] = img[cut_y - bot_shift:h - bot_shift, cut_x - right_shift:w - right_shift]
        out_img1[cut_y:, cut_x:] = img1[cut_y - bot_shift:h - bot_shift, cut_x - right_shift:w - right_shift]

    return out_img,out_img1, bboxes
...
class Yolo_dataset(Dataset):
    def __init__(self, label_path,dataset_dir0,dataset_dir1, cfg, train=True):
        ...
        self.cfg = cfg
        self.train = train
        self.dataset_dir0=dataset_dir0
        ...
        # self.truth={path1:[box1,box2...]}
        self.truth = truth
        self.imgs = list(self.truth.keys())#路径名列表
    ...
    def __getitem__(self,index):
        ...
        img_path0 = os.path.join(self.dataset_dir0, img_path.split('/')[-1])
        img_path1 = os.path.join(self.dataset_dir1, img_path.split('/')[-1])#.replace('.jpg','.png'))
        ...
        out_img0 = np.zeros([self.cfg.h, self.cfg.w, 3])
        out_img1 = np.zeros([self.cfg.h, self.cfg.w, 3])
        out_bboxes = []
        for i in range(use_mixup + 1):
            ...
            if (min_w_h / 8) < blur and blur > 1:  # disable blur if one of the objects is too small
                blur = min_w_h / 8
            ai,ai1 = image_data_augmentation(img0,img1, self.cfg.w, self.cfg.h, pleft, ptop, swidth, sheight, flip,
            ...
            if use_mixup == 1:
                if i == 0:
                    ...
                    out_img0 = cv2.addWeighted(ai, 0.5, old_img, 0.5)
                    out_img1 = cv2.addWeighted(ai1, 0.5, old_img1, 0.5)
                ...
                out_img0,out_img1, out_bbox = blend_truth_mosaic(out_img0,out_img1,ai, ai1, truth.copy(), self.cfg.w, self.cfg.h, cut_x,
        ...
        out_img=np.concatenate((out_img0,out_img1[:,:,0].reshape(out_img1.shape[0],out_img1.shape[1], 1)),axis=2)
